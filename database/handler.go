...
func toDoc(document *deploy.Document) types.DocData {
	...
	return types.NewDocData(document.Id, data)
}

func (d handler) forEachDoc(documents *deploy.Documents, cb func(data types.DocData)) (*deploy.Ids, error) {
	var ids *deploy.Ids
	var err error
...
func (d handler) New(ctx context.Context, documents *deploy.Documents) (*deploy.Ids, error) {
	return d.forEachDoc(documents, func(doc types.DocData) {
		d.Database.New(doc)
	})
}
...
func (d handler) Get(ctx context.Context, ids *deploy.Ids) (*deploy.Documents, error) {
	...
	for _, id := range ids.Id {
		if doc := d.Database.Get(id); doc != nil {
			data, _ := json.Marshal((*doc).DataJson())
